...
exports.createOutputGuard = createOutputGuard;
const disclaimers_1 = require("./disclaimers");
const FINANCE_BLOCKED = [
...
function checkCalorieRecommendations(output) {
 ...
 * ```
 */
function createOutputGuard(config) {
    const { domain, severity = 'disclaim', disclaimerLength = 'short', appendDisclaimer = true, extraBlockedPatterns = [], } = config;
    const domainPatterns = DOMAIN_PATTERNS[domain] || [];
    const allPatterns = [
        ...domainPatterns,
        ...extraBlockedPatterns.map(p => ({ ...p, severity: 'block' })),
    ];
    return (output) => {
        const warnings = [];
        ...
        if (blocked && severity === 'block') {
            ...
            const disclaimer = (0, disclaimers_1.getDisclaimer)(domain, disclaimerLength);
            finalOutput = `${output}\n\n_${disclaimer}_`;
